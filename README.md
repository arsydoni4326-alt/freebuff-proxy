...
**Where dashboard token changes persist.** Token add/remove/swap/move mutations from `/admin` are applied live and written to the SQLite token database when it is active (`AUTH_TOKEN_DB_PATH`, default `data/auth_tokens.db`) and otherwise to `.env`. When the proxy runs with `-config <file>`, the new `AUTH_TOKENS` array is additionally mirrored back into that JSON file (key order and unrelated keys are preserved byte-exact), so a `-config /app/config.json` deployment survives container recreates and restarts. If the file cannot be written (e.g. a read-only or single-file bind mount), the mutation still applies to `.env`/the database and a `token change not mirrored into -config file` warning is logged.
...
- **Tokens**: pooled credentials as `Account #1, #2, …` (1-based pool order) with live cooldown countdowns, at-risk account cards, per-account **Move Up/Down** reorder, **Clear**/**Lock**/**Unlock**/**Remove**, expandable session drawer (**Drop Session**, model-lock pinning), rotation-policy radios, 429 auto-failover switch, **Add Token to Pool**, and the headless OAuth login wizard. Add/remove/swap/move persist to the active config source — `.env`, the `-config` JSON file when the proxy runs with one, and/or the SQLite token database (see [Configuration Reference](#configuration-reference)).
