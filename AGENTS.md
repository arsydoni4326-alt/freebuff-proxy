...
- `main` = production. The ONLY prod box is the small VPS reachable via the
  `production` ssh host (addresses live in local ssh config, never in repo),
  ...
  artifacts only: build the docker image on a review host-local, transfer it,
...
- Testing happens on the dev box, the homelab reachable via the
  `a review host-local` ssh host over LAN: preview builds and bugfix
...
- Cancelled remote builds leave orphaned `go compile` processes holding RAM:
  ...
  retrying. Keep production disk under 80% (`docker builder prune`, no dev
