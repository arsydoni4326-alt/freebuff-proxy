...
type rawConfig struct {
	...
	// exhaustion (EXHAUSTION_WARNING_THRESHOLD; "" = disabled).
	ExhaustionWarningThreshold string `json:"EXHAUSTION_WARNING_THRESHOLD"`
	// HealthScoreEnabled gates the composite 0-100 health score
	// (HEALTH_SCORE_ENABLED; default true).
	HealthScoreEnabled bool `json:"HEALTH_SCORE_ENABLED"`
	// TokenHealthProbes runs background zero-cost liveness probes per token
	// (TOKEN_HEALTH_PROBES; default false).
