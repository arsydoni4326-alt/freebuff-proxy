...
type rawConfig struct {
	...
	// BridgeCircuitBreakerFailures is the number of consecutive failures
	// before the circuit breaker opens (BRIDGE_CIRCUIT_BREAKER_FAILURES; 0 = disabled).
	BridgeCircuitBreakerFailures     *int                    `json:"BRIDGE_CIRCUIT_BREAKER_FAILURES"`
	// BridgeCircuitBreakerWindow is the time window for counting failures
	// (BRIDGE_CIRCUIT_BREAKER_WINDOW; e.g. "1m", "" = disabled).
	BridgeCircuitBreakerWindow       string                  `json:"BRIDGE_CIRCUIT_BREAKER_WINDOW"`
	// BridgeCircuitBreakerCooldown is the cooldown period after the breaker
	// opens before trying again (BRIDGE_CIRCUIT_BREAKER_COOLDOWN; e.g. "30s", "" = disabled).
...
func defaultRawConfig() rawConfig {
	return rawConfig{
		...
		QuotaProbeActiveInterval:         "60s",                        // reserved mirror of the upstream smartprobe knob surface (autoprobe uses its own schedule)
		QuotaProbeIdleHeartbeat:          "30m",                        // reserved mirror of the upstream smartprobe knob surface
		WaitingRoomChain:                 false,                        // waiting room disabled by default (issue #180): enables chain admission when true
		RateLimitPerIP:                   nil,                          // 0 = unlimited per-IP rate: DefaultRateLimitPerIP handles nil
		RateLimitBurst:                   nil,                          // 0 = unlimited burst
		...
		BridgeRateLimitPerToken:          nil,                          // 0 = unlimited per-token bridge rate
		BridgeCircuitBreakerFailures:     nil,                          // 0 = circuit breaker disabled
		BridgeCircuitBreakerWindow:       "",                           // "" = disabled
		BridgeCircuitBreakerCooldown:     "",                           // "" = disabled
		AutoRotateOnExhaustion:           false,                        // auto-rotate on exhaustion off by default (issue #274)
		ExhaustionWarningThreshold:       "",                           // "" = disabled
		HealthScoreEnabled:               false,                        // health score off by default (issue #281)
		TokenHealthProbes:                false,                        // token health probes off by default (issue #281)
		TokenProbeInterval:               "",                           // "" = disabled
		ModelLocks:                       "",                           // empty = no model locks (issue #325)
		CompressPrompt:                   "",                           // "" = disabled (unset → SAFE_MODE preset may fill)
		CacheControlInjection:            "",                           // "" = disabled (unset → SAFE_MODE preset may fill)
		ReasoningInContent:               "",                           // "" = disabled (unset → SAFE_MODE preset may fill)
	}
}
