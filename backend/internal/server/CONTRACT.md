...
- Token-mutation persistence: every dashboard token add/remove/swap/move (and the login-wizard add) funnels through `syncTokensAfterMutation` — SQLite token DB when active, else `.env` — with a persist → reload-verify → rollback contract that restores the file byte-exact on failure. The `-config` JSON file is NEVER rewritten (it is read-only input; a bind-mounted config.json must not be touched). At startup `cli.Serve` reconciles the pool to the DB's authoritative token list and re-applies the persisted per-token state (admin locks + terminal quarantines via `pool.TokenStateStore`, stored in the DB's `token_state` table) so locked/dead accounts stay locked/quarantined across restarts.
