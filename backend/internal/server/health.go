...
func (s *Server) handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	sb.WriteString("# HELP freebucks_proxy_allowlist_skips_total Acquire-time model-allowlist skips per token (MODEL_LOCKS)\n")
	sb.WriteString("# TYPE freebucks_proxy_allowlist_skips_total counter\n")
	for _, snap := range snaps {
		if snap.AllowlistSkips > 0 {
			fmt.Fprintf(&sb, "freebucks_proxy_allowlist_skips_total{token=\"%d\"} %d\n",
	...
	sb.WriteString("# HELP freebucks_proxy_registry_age_seconds Seconds since the last successful live model registry refresh (0 = never refreshed)\n")
	sb.WriteString("# TYPE freebucks_proxy_registry_age_seconds gauge\n")
	fmt.Fprintf(&sb, "freebucks_proxy_registry_age_seconds %g\n\n", registryAge)
	sb.WriteString("# HELP freebucks_proxy_registry_fallback 1 when the registry is serving the offline hardcoded fallback, 0 when live-refreshed\n")
	sb.WriteString("# TYPE freebucks_proxy_registry_fallback gauge\n")
	fmt.Fprintf(&sb, "freebucks_proxy_registry_fallback %d\n\n", registryFallback)

	// Deprecated: every core family above is re-emitted below under the
	// pre-rename product prefix, so existing scrapers and dashboards keep
	// resolving during the transition. Same bodies — one emit path, no
	// second copy to keep in sync. The merge-guarded fork families above
	// (bridge_*/registry_*/breaker_*) already carry the pre-rename prefix.
...
func (s *Server) bridgeMetrics(sb *strings.Builder) {
	...
	sb.WriteString("# HELP freebucks_proxy_bridge_entries_total Current bridge cache entries\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_entries_total gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_entries_total %d\n\n", len(bridgeSnaps))
	...
	sb.WriteString("# HELP freebucks_proxy_bridge_cooling_down_total Bridge entries currently cooling down\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_cooling_down_total gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_cooling_down_total %d\n\n", coolingDown)

	sb.WriteString("# HELP freebucks_proxy_bridge_dead_tokens_total Bridge entries with dead tokens\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_dead_tokens_total gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_dead_tokens_total %d\n\n", deadTokens)

	sb.WriteString("# HELP freebucks_proxy_bridge_locked_total Bridge entries locked by administrator\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_locked_total gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_locked_total %d\n\n", locked)

	sb.WriteString("# HELP freebucks_proxy_bridge_requests_total Total requests served through bridge entries\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_requests_total counter\n")
	for _, snap := range bridgeSnaps {
		fmt.Fprintf(sb, "freebucks_proxy_bridge_requests_total{token_label=\"%s\"} %d\n",
			escapeLabelValue(snap.Key[:min(8, len(snap.Key))]), snap.Requests)
	}
	sb.WriteString("\n")

	sb.WriteString("# HELP freebucks_proxy_bridge_active_runs Active agent runs per bridge entry\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_active_runs gauge\n")
	for _, snap := range bridgeSnaps {
		fmt.Fprintf(sb, "freebucks_proxy_bridge_active_runs{token_label=\"%s\"} %d\n",
			escapeLabelValue(snap.Key[:min(8, len(snap.Key))]), snap.ActiveRuns)
	}
	sb.WriteString("\n")

	sb.WriteString("# HELP freebucks_proxy_bridge_quota_remaining Quota remaining per bridge entry per model\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_quota_remaining gauge\n")
	for _, snap := range bridgeSnaps {
		for model, q := range snap.QuotaByModel {
			rem := float64(0)
			if q.Limit > 0 {
			...
			fmt.Fprintf(sb, "freebucks_proxy_bridge_quota_remaining{token_label=\"%s\",model=\"%s\"} %g\n",
	...
	sb.WriteString("# HELP freebucks_proxy_bridge_breaker_open 1 when the circuit breaker is blocking requests, 0 otherwise\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_breaker_open gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_breaker_open %d\n\n", openVal)

	sb.WriteString("# HELP freebucks_proxy_bridge_breaker_failures Current number of transient failures in the circuit breaker sliding window\n")
	sb.WriteString("# TYPE freebucks_proxy_bridge_breaker_failures gauge\n")
	fmt.Fprintf(sb, "freebucks_proxy_bridge_breaker_failures %d\n\n", breakerSnap.FailureCount)
}
