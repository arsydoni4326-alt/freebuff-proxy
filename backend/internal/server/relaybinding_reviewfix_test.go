...
func newRelayBindingServer(t *testing.T) *Server {
	...
	convert.SetReasoningLookup(s.reasoningCache.Get)
	t.Cleanup(func() { convert.SetReasoningLookup(nil) })
	return s
}

func normalizeAssistantEcho(t *testing.T, content any, toolCalls []map[string]any) map[string]any {
	t.Helper()
	...
	out, err := convert.NormalizeRequest(b, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRelayBindingOpenAIStreamRoundTrip(t *testing.T) {
	...
	t.Run("echo of the same calls with content null restores", func(t *testing.T) {
		m := normalizeAssistantEcho(t, nil, []map[string]any{bindingToolEcho("get_weather", `{"city":"SF"}`)})
	...
	t.Run("same ids with different arguments restore nothing", func(t *testing.T) {
		m := normalizeAssistantEcho(t, nil, []map[string]any{bindingToolEcho("get_weather", `{"city":"NYC"}`)})
	...
	t.Run("same ids with a different tool name restore nothing", func(t *testing.T) {
		m := normalizeAssistantEcho(t, nil, []map[string]any{bindingToolEcho("other_tool", `{"city":"SF"}`)})
		if rc, exists := m["reasoning_content"]; exists {
			if str, isStr := rc.(string); isStr && str != "" {
...
func TestRelayBindingNonStreamJSONRoundTrip(t *testing.T) {
	...
	t.Run("client echo with whitespace-variant arguments binds", func(t *testing.T) {
		m := normalizeAssistantEcho(t, "text before tool",
			[]map[string]any{bindingToolEcho("get_weather", `{"city":"SF"}`)})
	...
	t.Run("foreign arguments still rejected", func(t *testing.T) {
		m := normalizeAssistantEcho(t, "text before tool",
			[]map[string]any{bindingToolEcho("get_weather", `{"city":"NYC"}`)})
		if rc, exists := m["reasoning_content"]; exists {
			if str, isStr := rc.(string); isStr && str != "" {
				t.Fatalf("expected no restore for foreign arguments, got %q", str)
			}
