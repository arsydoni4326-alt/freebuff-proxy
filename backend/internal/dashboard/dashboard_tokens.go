...
func (d *Dashboard) tokensLiveData() tokensLiveData {
	...
	live.BridgeTokenCards = d.bridgeCards(showBridge, cfg.MaxSpendPerDay)
	for _, t := range d.pool.Snapshot() {
...
func (d *Dashboard) bridgeCards(show bool, spendLimit int64) []bridgeTokenCard {
	if !show {
		return nil
	}
	var out []bridgeTokenCard
	for _, snap := range d.pool.BridgeSnapshot() {
		out = append(out, bridgeCardFromSnapshot(snap, spendLimit))
	}
	return out
}
