...
* Hermetic `env -u AUTH_TOKENS -u ADMIN_TOKEN go test ./backend/...` `ok`, `sync-upstream --check` `exit 0` (`registry` + `wire` SAMEs), SG `127.0.0.1` `no stealth` logs, `~80-111 tok/s` streaming, `header 2.5-10.6s` upstream-dominated.
...
* **Gate 3 — Live SG** `127.0.0.1:3457` `curl -N POST /v1/chat/completions stream:true` timestamp per `data:` → `header 2.5-10s` + `~80-111 tok/s` (`tokens≈chars/4`), `docker logs | grep stealth` `0` lines (plain), `GET /v1/models` 6 available vs 19 registry expected (tier, not TLS).
