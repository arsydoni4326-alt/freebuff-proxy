...
|---|---|---|
| `FREEBUFF_MODELS` | CLI/Desktop picker — nine rows in pick order: GLM 5.3 Flash, DeepSeek V4 Flash, GPT-6 Luna, MiMo 2.6 Flash, MiMo 2.6 Pro, Solar Mini 4, Space Bunny Alpha, Gemini 3.8 Flash, Muse Spark 1.2. Solar Mini 4 took Solar Pro 4's slot on 2026-09-23 at half Pro 4's Freebucks price, and the BETA stealth row Space Bunny Alpha joined after the named-vendor rows (a row carrying the stealth caveat must not outrank ones without it). GPT-5.6 Luna left this list on 2026-09-22 and Solar Pro 4 on 2026-09-23 (retired-from-picker ids, not paused ones — §9.3). The two MiMo rows ride a `...(FREEBUFF_ENABLE_MIMO_MODELS_IN_UI ? [MIMO_V25_MODEL, MIMO_V26_PRO_MODEL] : [])` spread (flag `true`, `:1049`; spread `:2404-2406`); the two Pro-only rows are drawn locked, never hidden — so is GPT-6 Luna for a non-US viewer without a plan (§9.5) | `common/src/constants/freebuff-models.ts:2355-2460` |
...
| `SUPPORTED_FREEBUFF_MODELS` | 17 recognised rows — the picker rows **plus** paused, withdrawn and retired-from-picker ids, kept so released binaries hold ids the server can coerce rather than refuse. Solar Mini 4 and Space Bunny Alpha joined it on 2026-09-23; GPT-5.6 Luna and Solar Pro 4 stay in it as retired-from-picker ids | `:2303-2326` |
...
- `supersededBy` is declared on the interface and read by the picker nudge (`:4284-4297`), and Solar Pro 4 sets it — the first live notice since 2026-08-21: modelId Solar Mini 4, `'Solar Mini 4 replaces Solar Pro 4'`, action `'Switch to Solar Mini 4'`, migrating a saved Pro 4 pick on next read (`:1876-1880`).
...
| `upstage/solar-mini4` | OpenRouter, endpoint `upstage`, `allow_fallbacks:false` — the same Upstage lane as Pro 4 | UNLIMITED (`premium` from the entitlement = false); takes Pro 4's `LIMITED` slot | none — the route exposes no effort parameter | half Pro 4's Freebucks price (slot comment) | no | tagline `'Fast and light'`; `isNew`; Solar Pro 4 left every picker on 2026-09-23 but stays SUPPORTED and admissible with a `supersededBy` nudge to this row, so saved picks migrate instead of refusing (`:1905-1916`, `:2416-2419`, `:1876-1880`, `common/src/constants/freebuff-model-entitlements.ts:19-23`) |
| `stealth/space-bunny-alpha` | OpenRouter stealth endpoint, default route like Ox Alpha, zero-price fence (`FREEBUFF_SPACE_BUNNY_ALPHA_MAX_PRICE` prompt/completion 0) | STANDARD, `premium:false` — sits in no premium pool; the Freebucks price is the meter | through-max (`EFFORTS_THROUGH_MAX`) → high | 10 | yes (text+image+video in) | BETA (`experimental`) + `isNew`; `warning` = `'Anonymous provider retains prompts'` (`dataUse:'service'`, keeps prompts, does not train); tagline `'1M context'` with the stealth tooltip; placed after the named-vendor rows on purpose (`:2281-2301`, `:804-843`) |
...
Cites for 9.2 are `common/src/constants/freebuff-models.ts` unless another path is given. Prices in the last-and-second-last columns for Luna/MiMo/Flash/Bunny are test fixtures around the real per-session map; only GLM's "5 on every tier", Solar's schedule, and Mini 4's half-of-Pro-4 slot price are stated in source prose.

- Context windows drive the CLI's compaction budgets (`FREEBUFF_MODEL_CONTEXT_WINDOWS`, `:1297-1348`): DeepSeek V4 Flash/Pro 1,048,576 · GLM 5.3 Flash 1,000,000 · GPT-6 Luna 1,000,000 · GPT-5.6 Luna 1,000,000 (Luna-ES 372,000) · Muse Spark 1.2 1,000,000 · Ox Alpha 1,000,000 · Solar Mini 4 500,000 · Space Bunny Alpha 1,000,000 · Solar Pro 4 500,000 (retired row keeps its entry) · MiniMax M3 524,288; every other id (MiMo included) falls back to `FREEBUFF_DEFAULT_CONTEXT_WINDOW = 131,072` (`:1351`). Published limits are entered deliberately low; only Flash/Pro were read off a provider rejection.

### 9.3 Paused, retired, withdrawn

Cites in this subsection are `common/src/constants/freebuff-models.ts`.

- `FREEBUFF_PAUSED_FREE_MODEL_IDS` (`:2540-2656`), in order: Muse Spark 1.3 (2026-09-07, 404 on every key), MiniMax M3 (08-20, largest single bill line), DeepSeek V4 Pro (08-26, cost), Ox Alpha (08-27, host ended the promo), GLM 5.2 (08-31, reward moved). **GPT-5.6 Luna is deliberately not in it** (`:2541-2543`).
- GPT-5.6 Luna is the swap's other half: retired from every picker on 2026-09-22 (it left `FREEBUFF_MODELS`) but still recognised and **admissible** — sessions admitted before the swap drain on it, its agents stay bundled, and server machinery outside this package still resolves the id. That is stage one of a retirement; pausing is stage two and its prerequisite is recorded with that machinery, not here (`:1736-1766`, `:2303-2326`, `:2541-2543`). The slot comment inside `FREEBUFF_MODELS` calls it paused (`:2400-2403`) and that is wrong — the pause list's own header says explicitly that it is not. Solar Pro 4 mirrors it since 2026-09-23: retired from every picker (slot taken by Solar Mini 4) but still recognised and admissible, with a `supersededBy` nudge to Mini 4 (`:2416-2419`, `:1876-1880`).
...
- base2 root per model — `FREEBUFF_ROOT_AGENT_ID_BY_MODEL`, 27 entries (`common/src/constants/free-agents.ts:445-478`): Flash → `base2-free-deepseek-flash`, GLM 5.3 Flash → `base2-free-glm-5-3-flash`, Luna → `base2-free-luna` (GPT-5.6) with GPT-6 Luna on its own `base2-free-luna-6`, MiMo → `base2-free-mimo`, Solar Pro 4 → `base2-free-solar-pro4` with Solar Mini 4 on its own `base2-free-solar-mini4`, Space Bunny Alpha → `base2-free-space-bunny-alpha`, Muse Spark 1.2 → `base2-free-muse-spark`, Fable 5.1 → `base2-free-fable`; unknown models fall back to `base2-free`. Each swap mints new ids rather than reusing the retired row's, so spend and run counts stay split.
- base3 (single-loop harness) has two per-surface maps whose ids are deliberately shared: Web 18 entries (`:133-154`) and CLI 16 entries (`:174-197`), both carrying `base3-free-luna-6` beside 5.6's `base3-free-luna` and the new `base3-free-solar-mini4` / `base3-free-space-bunny-alpha` beside `base3-free-solar-pro4`. `getFreebuffBase3RootAgentIdForModel` falls back to the model's **base2** root, never another model's base3 root (`:569-572`).
- Reviewer per model: `FREEBUFF_REVIEWER_AGENT_ID_BY_MODEL` (`:494-521`, 17 entries — GPT-6 Luna → `code-reviewer-luna-6`, 5.6 keeps `code-reviewer-luna`; Solar Mini 4 → `code-reviewer-solar-mini4`, Bunny → `code-reviewer-space-bunny-alpha`); every entry must run the same model as its key, because the chat-completions session gate 403s `session_model_mismatch` on a cross-model reviewer (`:468-477`).
...
| limited tier | `LIMITED_FREEBUFF_MODELS` = GLM 5.3 Flash (hero), Flash, MiMo, Solar Mini 4 (`:3286-3296`, Solar Mini 4 took Solar Pro 4's place on 2026-09-23); hero is `LIMITED_FREEBUFF_HERO_MODEL_ID` = GLM 5.3 Flash (`:3260-3261`); the **coercion target** is a different row, `LIMITED_FREEBUFF_MODEL_ID` = Flash, chosen because it is joinable with no meter, no grant and no plan (`:3248-3249`) |
...
- Ladders, all `as const` in `common/src/constants/freebuff-models.ts`: DeepSeek V4 `['low','high','max']` (`:832`), GLM 5.3 Flash `['low','high','max']` (`:917`), Luna — GPT-6 and GPT-5.6 alike — through-max (`:808-813`), Muse Spark through-xhigh (`:801-806`), Ox Alpha `['low','high','max']` (`:837`). MiMo, Solar Mini 4 and MiniMax M3 carry no ladder because their routes expose no native effort parameter. Space Bunny Alpha runs `EFFORTS_THROUGH_MAX` → high (`:2295-2296`).
